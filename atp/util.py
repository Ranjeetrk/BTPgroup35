...
from atp.article import Article
from atp.Prepostion_Article import Article as prep_article
from atp.Verb_Article import Article as verb_article
...
def generate_trivia_2(filename):
    # Retrieve the trivia sentences
    questions = list()
    # create an object
    obj_a = prep_article(filename)
    # call method on the object
    questions.append(obj_a.generate_trivia_sentences())
    # list to store que and ans in the form of a dictionary
    que_ans_pair = list()
...
def generate_trivia_3(filename):
    # Retrieve the trivia sentences
    questions = list()
    # create an object
    obj_a = verb_article(filename)
    # call method on the object
    questions.append(obj_a.generate_trivia_sentences())
    # list to store que and ans in the form of a dictionary
    que_ans_pair = list()
...
def relative_ranking(subjectname, flag):
    ...
    max_score = max(temp_df["score_ob"])
    min_score = min(temp_df["score_ob"])
    mean_score = temp_df["score_ob"].mean()
